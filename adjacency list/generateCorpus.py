...
# read in our training data, removing punctuation.
words = makepairs(infile.read().
                  translate(str.maketrans('', '', string.punctuation)).split())

# this loop counts the number of times word B follows word A
# every pair of words is counted as a tuple (pair) and the value of dictionary at
# (a,b) is incremented
for word in words:
    if word[0] in corpus:
...
# this is the part where we go through every value in the corpus and translate number of
# occurrences to a probability out of 1
for words in corpus:
    counter = 0
...
# lastly we format the output in a way that isn't a pain in the ass for c++ to parse
# (jk, it's always a pain in the ass to parse file input in c++)
